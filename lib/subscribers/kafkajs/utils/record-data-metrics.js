...
module.exports = function recordDataMetrics({ data, kafkaCtx, tx }) {
  ...
  const rawPsn = data?.message?.headers?.producerServiceName
  if (rawPsn) {
    const producerServiceName = Buffer.isBuffer(rawPsn) ? rawPsn.toString() : String(rawPsn)
    tx.trace.attributes.addAttribute(
      DESTINATIONS.TRANS_EVENT,
      'kafka.producer.serviceName',
      producerServiceName
    )
    tx.trace.attributes.addAttribute(
      DESTINATIONS.TRANS_EVENT,
    ...
    metrics
      .getOrCreateMetric(`Message/Kafka/Topic/Named/${topic}/Producer/${producerServiceName}`)
      .incrementCallCount()
  }
}
